...
from .config import BASE_CONFIG_DIR, DB_PATH, upload_path
...
def dashboard():
        ...
        for tenant_dir in BASE_CONFIG_DIR.iterdir():
            if tenant_dir.is_dir():
                ...
                store_exists = (
                    "✅"
                    if store_path(
                        tenant_dir.name, agents[0] if agents else "default"
                    ).exists()
                    else "❌"
                )
    ...
    def show_users():
        while True:
            ...
            for username, user_data in users_db.items():
                status = (
                    "❌ Disabled" if user_data.get("disabled", False) else "✅ Active"
                )
                table.add_row(
                    username,
            ...
            if choice == "1":
                ...
                if create_user(user_data):
                    console.print(
                        f"[green]✅ User '{username}' created successfully[/green]"
                    )
                else:
    ...
    def show_status():
        ...
        google_creds = (
            "✅ Set" if os.getenv("GOOGLE_APPLICATION_CREDENTIALS") else "❌ Missing"
        )
...
def cli_ingest(
    ...
    drive: Optional[str] = typer.Option(
        None, "--drive", "-d", help="Google Drive folder ID"
    ),
    files: Optional[List[str]] = typer.Option(
        None, "--file", "-f", help="Local file paths"
    ),
):
    ...
    if not any([sitemap, drive, file_paths]):
        up_dir = upload_path(tenant, agent)
        if up_dir.exists():
            file_paths = [p for p in up_dir.iterdir() if p.is_file()]
        else:
            console.print(f"[red]No files found in {up_dir}[/red]")
            raise typer.Exit(1)
...
def create_user_cli(
    ...
    agents: Optional[List[str]] = typer.Option(
        None, help="Agent names (comma separated)"
    ),
):
    """Create a new user"""
